    ...
    for (int i = 0; i < header.PathDataCount; i++)
    {
        ...
        if (json_object_object_get_ex(pathDataObj, "LinkedNodes", &linkedNodes))
        {
            for (int j = 0; j < 4; j++)
            {
                ...
                {
                    // strncpy(pathData.LinkedNodes[j], json_object_get_string(json_object_array_get_idx(linkedNodes, j)), 4);
